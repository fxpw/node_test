...
const ProjectController = {
	...
	updateOrder: async (req, res) => {
		try {
			...
			let updatePromises = updatedProjects.map(project =>
				projectService.updateProject(project.id, { order: project.order })
			);

			let results = await Promise.all(updatePromises);

			// Проверяем, все ли проекты были обновлены
			if (results.every(result => result !== null)) {
				res.status(200).json({ message: 'Порядок проектов обновлен!' });
			} else {
				res.status(404).json({ message: 'Некоторые проекты не были найдены для обновления.' });
			}
